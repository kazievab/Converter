...
class JapgollyGenComponents(
  ...
  /**
    ...
    *
    * This is primarily to support React components exported as plain generic
    * functions like `declare const List: <T>(props: ListProps<T>) => ReactElement`.
    * In such cases the generic `T` lives only on the function type, not on a
    * surrounding class/value, so our normal component discovery doesn't see it.
    */
